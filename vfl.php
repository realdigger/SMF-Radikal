...
$downloadFullsize = false; // Если true, заменять миниатюры и среднеразмерные на полноразмерные изображения
...
$logFile               = __DIR__ . '/vfl.log.' . date('Y-m-d') . '.txt';
...
<head>
    <meta charset="UTF-8">
    <title>Импорт изображений с VFL.RU в SMF</title>
</head>
<body>
';
}

echo '*** Импорт изображений с VFL.RU в SMF' . $phpEOL;
...
// Находим сообщения с изображениями с VFL.RU
$result = $smcFunc['db_query']('', '
					SELECT id_msg 
					FROM {db_prefix}messages
					WHERE body LIKE "%vfl.ru/ii/%"',
                               []
);
...
while ($row = $smcFunc['db_fetch_assoc']($result)) {
    ...
    // Находим все изображения с VFL.RU в этом сообщении
    $pattern      = '#(?P<url>https?:\/\/[^\.\/]*.?vfl\.ru\/ii\/.+)[^\d\w\/\.]#iU';
...
/**
 * Обработка каждой ссылки на VFL.RU в сообщении
 *
 * @param $url
 * @param $msgId
 * @return bool
...
function processUrl($url, $msgId)
{
    ...
    $host    = str_replace('.vfl.ru', '', strtolower(parse_url($url, PHP_URL_HOST)));
    $host    = str_replace('vfl.ru', '000', $host);
    ...
    if ($downloadFullsize) {
        $file = str_replace(['_s.', '_m.'], '.', $file);
        $url  = str_replace(['_s.', '_m.'], '.', $url);
    }

    $dir = __DIR__ . '/vfl/' . $host . $path;
    @mkdir($dir, 0777, true);

    // Загружаем файл
    if (is_dir($dir) && downloadFile($url, $dir . '/' . $file, $msgId)) {
        return $boardurl . '/vfl/' . $host . $path . '/' . $file;
    }
